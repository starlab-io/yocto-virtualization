...
exec docker run \
    ...
    -e BUILD_UID=${my_uid} \
    -e BUILD_GID=${my_gid} \
    -e TEMPLATECONF=meta-virtualization-intel/conf \
    -e MACHINE=${MACHINE:-intel-corei7-64} \
    ${SSH_AUTH_SOCK:+-e SSH_AUTH_SOCK="/tmp/ssh-agent/${SSH_AUTH_NAME}"} \
    ${SSTATE_MIRRORS:+-e SSTATE_MIRRORS="$SSTATE_MIRRORS"} \
    ${SSTATE_DIR:+-e SSTATE_DIR="$SSTATE_DIR"} \
    -v ${HOME}/.ssh:/var/build/.ssh \
    -v "${PWD}":/var/build:rw \
    ${SSH_AUTH_SOCK:+-v "${SSH_AUTH_DIR}":/tmp/ssh-agent} \
    ${SSTATE_MOUNT:+-v $SSTATE_MOUNT} \
